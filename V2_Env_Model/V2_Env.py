...
class RobotEnv(gym.Env):
    ...
    def getObservation(self):
        ...
        # Contacts
        obs.append(int(len(p.getContactPoints(self.robot, self.terrain))))
    ...
    def evaluateReward(self, action):
        ...
        if num_contacts >= 2:
            # Progress reward
            if progress > 0.1:
                reward += 10 * progress * (1 + 0.05 * self.times_moved_forwards)
            ...
            reward += 7.5 * np.tanh(1 - abs(angle_deviation) / 70)
        if num_contacts <= 3:
            reward -= 4 - num_contacts
        ...
        if self.step_count % 5 == 0:
            for _, info in self.joint_poses.items():
