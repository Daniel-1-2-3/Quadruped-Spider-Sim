...
class Movements:
    ...
    def applyDefinedGait(self, correction=0):
        ...
        def FL_J1_map():
            ...
            if correction < 10:
                f += float(np.radians(max(abs(correction), 10)))
            return [f, f, b, b] 
        def FR_J1_map():
    ...
    def setJoints(self, predefined_actions, needGroundedLegs, tolerance=0.01):
        ...
        for i in range(150):
            ...
            if len(moving_joints.items()) == 0:
                break
            if controller.getDistanceTarget() < 1.5:
    ...
    def set_boulders(self, num_boulders=2, size=128, radius=3, height=2):
        self.boulder_poses = []

        for _ in range(num_boulders):
            x, y = random.randint(-64, 64), random.randint(-64, 64)
            self.boulder_poses.append([x, y]) 
            
    def draw_boulders(self, num_boulders=30, size=128, radius=3, height=2):
    ...
    def convert_to_pybullet_coords(self, coords):
        ...
        for i in range(len(coords)):
            coords[i] = (coords_copy[i][1] - 64, 64 - coords_copy[i][0])
        return coords

if __name__ == "__main__":
    controller = Movements()
    nav = Navigate()
    print(controller.boulder_poses)
    nav.selection(controller.boulder_poses)
    
    controller.toTarget([nav.start, nav.goal])
